...
class UAGBSectionEdit extends Component {
	...
	componentDidMount() {
		console.log("Component did mount of edit block section");
		...
		const { imageMediaBackground } = this.props.attributes
	}
	...
	handleChange(media) {
		console.log("selected option background is ", media);
		const { imageMediaBackground } = this.props.attributes
		const { setAttributes } = this.props

		if (!media) {
			setAttributes({ imageMediaBackground: null })
			return
		}

		setAttributes({ imageMediaBackground: media.value })
	}
	...
	render() {
		...
		const listZips = JSON.parse(window.localStorage.getItem('listMediasBackground'));
		const optionZips = listZips && listZips.length ? listZips.map((item, index) => { return { value: item, label: item.title } }) : [];
		const optionZipsFilter = optionZips.filter(media => !media.label.includes("mp4"));
		console.log("optionZipsFilter", optionZipsFilter);
		// const listImage = optionZips?optionZips.filter(media=>!media.title.includes("mp4")):[];
		...
		return (
			<Fragment>
				...
				<Section tagName={tag} className={classnames(
					className,
					"uagb-section__wrap",
					`uagb-section__background-${backgroundType}`,
					`uagb-section__edit-${active}`,
					...
					<div className="uagb-section__overlay"></div>
					{url && <div
						className='uagb-section__image-wrap'
						style={{
